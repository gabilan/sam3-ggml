...
static __global__ void cpy_scalar_transpose(const char * cx, char * cdst, const int64_t ne,
    ...
    // int64: grid_x may exceed INT_MAX/TILE_DIM blocks (llama.cpp #25000).
...
cudaStream_t stream) {
    ...
    GGML_ASSERT(num_blocks <= INT_MAX);  // CUDA grid.x limit is 2^31-1
...
static void ggml_cpy_scalar_cuda(
    ...
    // Upstream llama.cpp #25000: oversized transposed grids fall back to the
    // scalar kernel instead of asserting (CUDA grid_y/grid_z are 16-bit).
    ...
    if (transposed) {
        ...
        GGML_ASSERT(grid_x <= INT_MAX);  // CUDA grid.x limit is 2^31-1
