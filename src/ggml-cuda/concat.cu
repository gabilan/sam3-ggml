...
// contiguous kernels (element-type generic; F32 path unchanged in spirit)
template <typename T>
...
template <typename T>
static void concat_cont_cuda(const T * x, const T * y, T * dst, int ne00, int ne01, int ne02, int ne0, int ne1, int ne2, int dim, cudaStream_t stream) {
...
void ggml_cuda_op_concat(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // Scalar same-type concat (F16/BF16/F32/I*). Quantized types unsupported.
